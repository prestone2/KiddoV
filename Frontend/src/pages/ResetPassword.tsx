...
const ResetPassword = () => {
  ...
  if (initializing) {
    return (
      <div className="min-h-screen flex items-center justify-center px-4 bg-cover bg-center" style={{ backgroundImage: `url(${bgImage})` }}>
        <div className="max-w-md w-full bg-white rounded-2xl shadow-xl p-8 text-center">
          ...
          <p className="text-gray-600 mt-4">Please wait while we verify your link...</p>
        </div>
      </div>
    );
  }
  ...
  if (!hasValidSession) {
    return (
      <div className="min-h-screen flex items-center justify-center px-4 bg-cover bg-center" style={{ backgroundImage: `url(${bgImage})` }}>
        <div className="max-w-md w-full bg-white rounded-2xl shadow-xl p-8 text-center">
          <img src={logo} alt="Kiddovase" className="h-20 mx-auto rounded animate-bounce mb-4" />
          <h1 className="text-2xl font-bold text-gray-900 mb-4">Invalid Reset Link</h1>
          <p className="text-gray-600 mb-6">
            The reset link you used is invalid or has expired. You’ll be redirected shortly.
          </p>
          <Button onClick={() => navigate("/login")} className="w-full bg-gradient-to-r from-[#8d0b41] to-[#a60e4d] hover:from-[#750935] hover:to-[#8d0b41] text-white font-semibold rounded-full py-3 transition-all duration-200 shadow-lg">
            Go to Login
          </Button>
        </div>
      </div>
        ...
        >
          ...
          <form onSubmit={handleResetPassword} className="space-y-6">
            <div>
              ...
              <Input id="password" type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Enter your new password" className="mt-1" required minLength={6} />
            </div>
            <div>
              <Label htmlFor="confirm-password" className="text-gray-700">Confirm Password</Label>
              <Input id="confirm-password" type="password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} placeholder="Confirm your new password" className="mt-1" required minLength={6} />
            </div>
          ...
          <div className="mt-10 bg-roblox-blue text-white rounded-lg p-6">
            ...
            <p className="text-sm mb-4">
              If you’re unable to reset your password, visit our Help Center for assistance.
            </p>
